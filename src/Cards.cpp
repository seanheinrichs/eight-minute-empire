...
void Deck::exchange1(std::vector<Cards *> hand, Deck deck) {
    int position, index;
    cout << "Which card do you want to buy? Please enter an integer from 1 to 6: " << endl;
    cin >> position;
    while (position > 6 || position < 1) {
    ...
    index = position - 1;
    cout << "The card which shows: " << *topBoard.topBoard.at(index) << " and costs " << posArray[index] << " dollars." << endl;
    cout << "Would you like to buy this card ? " << endl;
    //TODO: call PayCoin() here
    hand.push_back(topBoard.topBoard.at(index));
    topBoard.topBoard.erase(topBoard.topBoard.begin()+index);
    cout << "Top Board cards series size is " << topBoard.topBoard.size() << endl ;
    topBoard.topBoard.push_back(deck.draw());
    cout << "Top Board cards series size is " << topBoard.topBoard.size() << endl ;
    cout << "Player's hand added a card shows " << *hand.back() << endl;
    cout << "Deck::exchange(hand, topboard, deck) works"  << endl;
}
...
Deck::TopBoard::TopBoard() {
    auto topBoard = new std::vector<Cards*>();
}
...
const vector<Cards *> &Deck::TopBoard::getTopBoard() const {
    return topBoard;
}

//void Deck::TopBoard::setTopBoard(const vector<Cards *> &topBoard) {
void Deck::TopBoard::setTopBoard(Deck deck) {
    auto tb = new std::vector<Cards*>();
    for (auto i = 0; i < 6; i++) {
        tb->emplace_back(deck.draw());
    }
