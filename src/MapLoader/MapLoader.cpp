...
{
  // possible maps
  std::vector<std::string> mapFiles = {"assets/american-revolution.map", "assets/korean-war.map", "assets/british-isles.map"};
  ...
  do
  {
    std::cout << "Select one of the scenarios below:\n";
    std::cout << "1) The American Revolution\n";
    std::cout << "2) The Korean War\n";
    std::cout << "3) The British Isles\n";
    std::cout << "Select from 1 to 3: ";

    // get choice
    std::cin >> choice;

    // break out of loop on valid selection
    if (choice >= 1 && choice <= 3)
    {
      validSelection = true;
    }

    // inform user on bad selection
    else
    {
      std::cout << "Invalid input. Please enter 1, 2 or 3.\n";
    }

    // if invalid character input, clear input and discard invalid characters
    // prevents infinite loop on some string inputs
    if (std::cin.fail())
    {
      std::cin.clear();
  ...
  return generateMap(mapFiles.at(choice - 1));
}
